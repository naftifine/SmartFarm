...
const controlButtonById = async (buttonId, status) => {
    ...
    if (!btn) throw new Error('Button does not exist');
    if (btn.status === status) throw new Error( `${btn.name} already has status: ${status}` );
    const payload = (status === 'On' || status === '1') ? '1' : '0';
